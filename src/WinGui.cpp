...
HWND hBtnStart = NULL;
HWND hBtnStop = NULL;
HWND hBtnAutoFix = NULL; // Yeni eklenen tamir butonu
...
extern DWORD RecvHookAddy; // Packetlogger'dan durum kontrolü için çekiyoruz

void UpdateUiElements()
{
    if (!pBotManager) return;

    // SENARYO 1: Eski baytlar patlamışsa menü kendini korumaya alır
    if (RecvHookAddy == NULL) {
        SetWindowTextA(hStatusLabel, "Durum: Adresler Hatalı! (Patch uyarısı)");
        EnableWindow(hBtnStart, FALSE); // Korumak için başlatmayı engelle
        EnableWindow(hBtnStop, FALSE);
        ShowWindow(hBtnAutoFix, SW_SHOW); // Otomatik bulma butonunu göster!
        return;
    }

    // SENARYO 2: Her şey yolundaysa normal menü akışı devam eder
    ShowWindow(hBtnAutoFix, SW_HIDE); // Tamir butonunu gizle
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_CREATE:
        ...
        // YENİ: Gizli otomatik adres bulma butonu (Sadece hata durumunda açılır)
        ...
        hDelaySlider = CreateWindowA("msctls_trackbar32", "", WS_CHILD | WS_VISIBLE | TBS_AUTOTICKS | TBS_HORZ, 20, 170, 230, 30, hwnd, (HMENU)103, NULL, NULL);
    ...
    case WM_COMMAND:
        ...
        // Butona tıklandığında hafızayı canlı tarayan tetikleyici:
        if (LOWORD(wp) == 200) { 
            if (Packetlogger::AutoFindAddresses()) {
                Packetlogger::HookRecv(); // Yeni bulunan adrese kancayı at
                MessageBoxA(hwnd, "Yeni hafıza adresleri başarıyla bulundu! Bot kullanıma hazır.", "Nova Suite Success", MB_OK | MB_ICONINFORMATION);
            } else {
                MessageBoxA(hwnd, "Otomatik tarama başarısız. Oyun köklü değiştirilmiş olabilir.", "Nova Suite Error", MB_OK | MB_ICONERROR);
            }
        }
        UpdateUiElements();
        break;
